...
var green, inquirer, fs, yaml, merge, N, path, tmplconv, yamlCutter, ref$, data, questions;
...
inquirer.prompt(questions).then(function(answers){
  ...
  srcPath = path.join(__dirname, '..', 'templates', 'add-service', data.templateName);
...
function serviceNames(){
  return fs.readdirSync(path.join(__dirname, '..', 'templates', 'add-service'));
}
function parseCommandLine(commandLineArgs){
  var data, questions, _, serviceName, templateName, modelName, description;
  data = {};
  questions = [];
  _ = commandLineArgs[0], _ = commandLineArgs[1], _ = commandLineArgs[2], serviceName = commandLineArgs[3], templateName = commandLineArgs[4], modelName = commandLineArgs[5], description = commandLineArgs[6];
  if (serviceName) {
